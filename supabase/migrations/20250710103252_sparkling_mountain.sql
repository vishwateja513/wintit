...
-- Insert sample audit assignments (only if we have users)
DO $$
DECLARE
    admin_user_id uuid;
...
BEGIN
    ...
    -- Get template IDs
    SELECT template_id INTO template1_id FROM audit_templates WHERE name = 'Comprehensive Store Audit' LIMIT 1;
    SELECT template_id INTO template2_id FROM audit_templates WHERE name = 'Quick Stock Check' LIMIT 1;
    ...
    -- Insert assignments only if we have the required data
    IF auditor_user_id IS NOT NULL AND supervisor_user_id IS NOT NULL AND template1_id IS NOT NULL THEN
...
BEGIN
    ...
    -- Insert widgets only if we have users
...
BEGIN
    ...
    SELECT user_id INTO auditor_user_id FROM user_profiles WHERE role = 'auditor' LIMIT 1;
    SELECT user_id INTO supervisor_user_id FROM user_profiles WHERE role = 'supervisor' LIMIT 1;
    SELECT assignment_id INTO assignment_uuid FROM audit_assignments LIMIT 1;
    
    -- Insert notifications only if we have users
    IF auditor_user_id IS NOT NULL THEN
        ...
        (
          ...
          'info',
          'assignment',
          jsonb_build_object('assignment_id', assignment_uuid)
        )
        ON CONFLICT DO NOTHING;
    END IF;
